...
class PFPascalDataset(CorrespondenceDataset):
    ...
    def __init__(self, benchmark, datapath, thres, split, augment, imside=240):
        r""" PF-PASCAL dataset constructor """
        super(PFPascalDataset, self).__init__(benchmark, datapath, thres, split, augment, imside)
        ...
        for src_imname, trg_imname, cls in zip(self.src_imnames, self.trg_imnames, self.cls_ids):
            ...
            for src_kk, trg_kk in zip(src_kp, trg_kp):
                if len(torch.nonzero(torch.isnan(src_kk).long())) != 0 or \
                        len(torch.nonzero(torch.isnan(trg_kk).long())) != 0:
                    continue
                else:
